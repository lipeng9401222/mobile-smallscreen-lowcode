...
const Config = {
    ...
    /**
     * 是否使用个性化定制目录
     * 开启后，代码在构建时将会响应customize目录下的个性化代码
     * 修改后需重启服务或重新构建
     */
    isCustomize: false,
    /**
    ...
    configReady: (jsApiList, successCallback, errorCallback) => {
        const tmpApiList = jsApiList.length > 0 ? jsApiList : process.env.MC_EJS_JSAPI_LIST ? process.env.MC_EJS_JSAPI_LIST :  [];
